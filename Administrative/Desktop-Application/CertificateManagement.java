...
{
	String[] demande = { "cmd.exe", "/C", "openssl req -newkey rsa:2048 -keyout C:\\E_docs_tool\\E_docs_Admin_tool\\privateKey_store\\myPrivateKey.pem -nodes -out C:\\E_docs_tool\\E_docs_Admin_tool\\request_store\\myReq.req -subj /CN="+matricule+"/O=USTHB/C=DZ/L=Alger" };
	String[] envoiDemande = { "cmd.exe", "/C", "certreq -submit -binary -attrib \"CertificateTemplate:SmartCardUserPFE\" -config WIN-NRUPPPRU6AV\\USTHB-WIN-NRUPPPRU6AV-CA C:\\E_docs_tool\\E_docs_Admin_tool\\request_store\\myReq.req C:\\E_docs_tool\\E_docs_Admin_tool\\certificat_store\\certificat.cer" };
	String[] privKey= {"cmd.exe","/C","openssl rsa -in C:\\E_docs_tool\\E_docs_Admin_tool\\privateKey_store\\myPrivateKey.pem -text -out C:\\E_docs_tool\\E_docs_Admin_tool\\privateKey_store\\myPrivateKey.txt"};
	try {
		ecran.cmd(demande,"");
		File req = new File("C:\\E_docs_tool\\E_docs_Admin_tool\\request_store\\myReq.req");
		while(!req.exists())
		{
		...
		Thread.sleep(1000);
		ecran.cmd(privKey,"");
File certif=new File("C:\\E_docs_tool\\E_docs_Admin_tool\\certificat_store\\certificat.cer");
	...
	try {
		...
		File cert=new File("C:\\E_docs_tool\\E_docs_Admin_tool\\certificat_store\\certificat.cer");
		...
		while((n1 = fis1.read(buf1)) >= 0)
	    {
	    	for(byte bit1 : buf1)
	    	{
	    		certificate[n2]=bit1; //ecriture du ficher certificat dans un tableau (tableau envoyé a la carte)
	    		n2++;
	    	}
	    }
		fis1.close();

...
public class CardManagement {
  ...
  //exposant privé pour RSA : d (String)
	...
	// les données utilisées pour le calcul de la clé AES	
...
/********************************************************************************************************/
	    protected static void evalSW(ResponseAPDU rep) // méthode pour évaluer la réponse de l'applet
	    {
	    	...
	    	case 0x6A83: JOptionPane.showMessageDialog(null,"Erreur : MoNtant de transaction invalide"+ rep.getSW(),"Problème !",JOptionPane.ERROR_MESSAGE);break;
...
/********************************************************************************************************/
	    protected static byte select() //début de la méthode select
	    {
	    	...
	    	try
			{
			    canal = c.getBasicChannel();//création d'un canal
				...
				{
	                JOptionPane.showMessageDialog(null, "Applet absente  " +selectReponse.getSW(), "Info !", JOptionPane.INFORMATION_MESSAGE);
	                return rep;
				}
				else
				{
...
/********************************************************************************************************/
				...
				msgAlClaire = decryptAuth(msgAlCrpt); // décrypter le message
					...
					{
					JOptionPane.showMessageDialog(null, "Problème lors l'opération de décryptage !", "Problème !", JOptionPane.ERROR_MESSAGE);
					return false;	
					}
...
/********************************************************************************************************/
				protected static void setAESKey() // demander les données de la carte pour calculer la clé AES
				{
					...
					try {
							...
							
						{
							...
							JOptionPane.showMessageDialog(null,"Problème lors la récupération de la clé AES");
							return;
							}
						}
...
/********************************************************************************************************/				
				 /*vérification de la carte par certificat+ signature crypté*/
				protected static boolean verifyCard() throws CardException , InvalidKeyException, SignatureException

				{
					
					CreatFile("C:\\Resto_tool\\Resto_Admin_tool\\certificat_store\\certificat.cer");
				...
				ResponseAPDU getInfoReponse = canal.transmit(getInfoApdu);
					...
					
					{
						...
						File file = new File("C:\\Resto_tool\\Resto_Admin_tool\\certificat_store\\certificat.cer");
                        ...
                        fop.flush();
                  		fop.close();
								...
								if(!CertificateManagement.verifyCerticat("C:\\Resto_tool\\Resto_Admin_tool\\certificat_store\\certificat.cer", "C:\\Resto_tool\\Resto_Admin_tool\\USTHB-CA.cer"))
								{
								...
								//Comparer les données et la signature
									...
									dsa.initVerify(CertificateManagement.getPublicKey("C:\\Resto_tool\\Resto_Admin_tool\\certificat_store\\certificat.cer"));
									 dsa.update(dataSign);
 ...
 * @throws NoSuchAlgorithmException ******************************************************************************************************/
	   ...
	   // méthode utilisée pour l'initialisations ou le renouvelement  
	    protected static boolean init(String PIN,String matricule,String nom,String prenom,String filiere,int type) throws CardException, NoSuchAlgorithmException, NoSuchPaddingException
	    {
	    	...
	    	RsaPrivateKeyMang clePriv= new RsaPrivateKeyMang("C:\\Resto_tool\\Resto_Admin_tool\\privateKey_store\\myPrivateKey.txt");
...
/********************************************************************************************************/
	    ...
	    protected static String getFiliere() throws IOException, CardException, NoSuchAlgorithmException, NoSuchPaddingException
	    {
	    	...
	    	System.out.println(ADMIN_CLA + "khra");

    		evalSW(getFilierReponse);
    		if (getFilierReponse.getSW() != 0x9000) 
			{
				JOptionPane.showMessageDialog(null, "Filière non récupérée"+" "+getFilierReponse.getSW());
				return null;
			}
			else
...
/********************************************************************************************************/
		   ...
		   // methode pour créer un ficher (utilisé pour créer un fichier qui contient le certificat de la carte)
...
/********************************************************************************************************/
		   // methode pour supprimer le contenu d'un dossier
...
/********************************************************************************************************/
		...
		// CardManagement.publicSpec = new RSAPublicKeySpec(CardManagement.modulo,CardManagement.rsaPublicKeyExponent);
		 ...
		 {
             JOptionPane.showMessageDialog(null,"Exception dans la méthode CardManagement(), problème lors le calcul des clés" + e.getMessage() , "Problème !", JOptionPane.ERROR_MESSAGE);
			 return ;
		 }
